...
[
  ...
  movement-limit      ;; Number of movement allowed for infected people
]
...
to move  ;; turtle procedure
  ifelse not infected? [
    ...
    if movement-limit > 0 [
      rt random-float 360
      fd 0.5
      set movement-limit movement-limit - 1
    ]
  ]
...
to choose
  ...
  if mouse-clicked? [
    let neighbor min-one-of turtles [distancexy mouse-xcor mouse-ycor]
    ask neighbor [
      set infected? true
      set susceptible? false
      set infection-length random recovery-time
